...
.compute_calibration_per_cause <- function(cif_at_horizon, pred_horizons,
  ...
  for (i in seq_along(pred_horizons)) {
    ...
    # Jitter only for neighbourhood smoothing to break ties; LOESS handles ties natively
    pred_jit   <- if (length(unique(pred_use)) < length(pred_use))
                    jitter(pred_use, factor = 1e-6)
                  else pred_use

    if (!loess_smoothing) {
      bw  <- if (is.null(bandwidth)) prodlim::neighborhood(pred_jit)$bandwidth
              else bandwidth
      nbh <- prodlim::meanNeighbors(x = pred_jit, y = pseudo_use, bandwidth = bw)
    ...
    # Note: for neighbourhood smoothing, error is computed over aggregated unique
    # x-values (nbh$uniqueX), so the length may be shorter than sum(keep).
    # For LOESS, error is computed over all individual observations. This
    # difference is intentional: neighbourhood smoothing aggregates by design.
...
#' @param pred_horizons Numeric vector of user-supplied prediction horizons
#'   (sorted, no NAs).  Used as labels in the output.
#' @param snapped_times Numeric vector of the same length as `pred_horizons`,
#'   giving the exact CIF time grid values each horizon was snapped to.  Used
#'   for indexing into `cif_arr`.
...
.compute_calibration <- function(cr,
  ...
  for (i in seq_along(causes)) {
    ...
    cif_at_horizon <- cif_arr[, i, horizon_idx]
