...
compute_metrics <- function(cr, eval_times = NULL,
  ...
  # Map each eval_times value to the nearest point on the CIF time grid.
  ...
  for (k in causes) {
    i      <- which(causes == k)
    nm     <- cause_nms[i]
    M      <- cif[, i, snap_idx, drop = FALSE]   # [n, length(eval_times)]
    ...
    if (comp_cidx_t) {
      cidx <- do.call(pec::cindex, c(
        list(object     = preds,
             formula    = f,
             data       = cr@data,
             eval.times = rep(tau, length(snapped_times)),
